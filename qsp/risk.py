...
def _check_sample_size(
    population: int, sample: int, rule: SamplingRule, alerts: List[RiskAlert]
):
    ...
    for dim, ds in report.dimension_stats.items():
        ...
        if dr < min_dim_coverage:
            ...
            _add_alert(
                ...
                lvl,
                "维度覆盖",
                f"维度 '{dim}' 覆盖率不足 ({dr * 100:.1f}%)",
                f"类别 {ds.total_categories}，覆盖 {ds.covered_categories}，"
                f"未覆盖: {ds.uncovered}",
                f"建议将 '{dim}' 纳入分层抽样的 stratify_by 参数。",
                dimension=dim,
                coverage_rate=dr,
                uncovered=ds.uncovered,
            )


def _check_small_batches(
    report: CoverageReport, alerts: List[RiskAlert], min_per_batch: int = 2
):
    ...
    if small:
        _add_alert(
            ...
            batches=small,
            threshold=min_per_batch,
        )


def _check_numerical_bias(
    report: CoverageReport, alerts: List[RiskAlert], bias_threshold: float = 0.2
):
    ...
    for col in cols:
        ...
        if relative_diff > bias_threshold:
            lvl = (
                RiskLevel.HIGH
                if relative_diff > bias_threshold * 2
...
def _check_method_vs_data(
    df: pd.DataFrame, rule: SamplingRule, alerts: List[RiskAlert]
):
...
def analyze_risks(
    ...
    _check_small_batches(
        report, alerts, min_per_batch=thresholds.get("min_per_batch", 2)
    )
    _check_numerical_bias(
        report, alerts, bias_threshold=thresholds.get("bias_threshold", 0.2)
    )
...
def risks_to_text(analysis: RiskAnalysis) -> str:
    ...
    lines.append(
        f"风险统计 - CRITICAL:{lc.get('CRITICAL', 0)}  HIGH:{lc.get('HIGH', 0)}  "
    ...
    for idx, alert in enumerate(analysis.alerts, 1):
        ...
        if alert.data:
            detail = ", ".join(
                f"{k}={v}" for k, v in alert.data.items() if not isinstance(v, list)
            )
            if detail:
